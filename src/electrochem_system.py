...
class measurements:
    ...
    def perform_EIS_experiment(self,
        ...
        logging.info(f"Waiting an extra {self.settle_time}s for temperature to settle..")
        ...
        for m in range(measurements):
            logging.info(f"[{id}] Measurement #{m}..")

            # Get start temperature for specific measurement
            ...
            # Build experiment from parameters
            ...
            # Get properties and plot
            results = self.get_impedance_properties(identifier=id, metadata=metadata)
  
            logging.info(f"[{id}] Ionic conductivity: {results[1]}mS/cm")
            ...
            with open(self.master_csv, 'a', newline='') as file:
                ...
                writer.writerow({
                    ...
                    'End OCP (V)': end_ocp,
                    'Total Measurement Time (mins)': round(end_time / 60, 2),
    ...
    def perform_CV_experiment(self,
        ...
        for m in range(measurements):
            ...
            result = self.get_electrochemical_stability_window(identifier=id, metadata=metadata)
